...
# NOTE: Use the service role key in your .env for backend scripts to allow inserts.
...
def load_student_list() -> List[str]:
    """Load the list of students to process from Supabase database"""
    students = []
    try:
    ...
    if not students:
        print("No students found in database")
        print("Please add students to the database students table")
        exit(1)
